...
try:
    ...
    cursor.execute(
        "CREATE TABLE PROFILES(USER VARCHAR(50) NOT NULL, PASSWORD VARCHAR(50) NOT NULL, EMAIL VARCHAR(50), EMAILPASS VARCHAR(50), DISCORD VARCHAR(20));")
    # Create Default User Journal
    cursor.execute("CREATE TABLE " + profileParams[
        0] + "_JOURNAL(DATE VARCHAR(50) NOT NULL, ENTRY MEDIUMTEXT NOT NULL, UUID VARCHAR(50), STARRED VARCHAR(50), CREATIONDEVICE VARCHAR(50), TIMEZONE VARCHAR(50));")

    # Create Table for saving Daily Briefings. Might be useful someday
    cursor.execute(
        "CREATE TABLE DAILY_BRIEFING(ID INT AUTO_INCREMENT PRIMARY KEY, DATE VARCHAR(50) NOT NULL, NEWS_LINKS MEDIUMTEXT, TECH_LINKS MEDIUMTEXT, TRENDING_SEARCHES MEDIUMTEXT);")
