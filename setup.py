...
try:
    ...
    cursor.execute("CREATE TABLE " + profileParams[0] + "-JOURNAL(DATE VARCHAR(50) NOT NULL, ENTRY MEDIUMTEXT NOT NULL, UUID VARCHAR(50), STARRED VARCHAR(50), CREATIONDEVICE VARCHAR(50), TIMEZONE VARCHAR(50));")
    # Create Default User Notes
    cursor.execute("CREATE TABLE " + profileParams[0] + "-NOTES(DATE VARCHAR(50) NOT NULL, TITLE MEDIUMTEXT, BODY MEDIUMTEXT NOT NULL, UUID VARCHAR(50), STARRED VARCHAR(50), CREATIONDEVICE VARCHAR(50), TIMEZONE VARCHAR(50));")
    db.commit()
except:
    # Rollback in case there is any error
    db.rollback()
