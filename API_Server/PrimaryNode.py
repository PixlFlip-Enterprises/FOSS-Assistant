...
class Journal(Resource):
    ...
    def get(self):
        ...
        # log
        Protocols.debug_log(console_printout="Journal View Entry", user=args['username'], command="000020", method_of_input="REST API")
        ...
        return {"date": "' + entry.date + '", "entry": "' + entry.entry + '", "starred":"' + entry.starred + '", "creation_device":"' + entry.creationDevice + '", "timezone":"' + entry.timeZone + '"}, 201

    def put(self):
        args = journal_put_args.parse_args()
        # log
        Protocols.debug_log(console_printout="Journal Create Entry", user=args['username'], command="000021", method_of_input="REST API")
        # todo add a password check for what should be obvious reasons
        PROFILE = User.Profile(args['username'])
